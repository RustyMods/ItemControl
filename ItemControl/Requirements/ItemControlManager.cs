...
public static class ItemControlManager
{
    ...
    public static string GetTooltip(string name, bool itemName)
    {
        ...
        {
            CraftRequirements.Add($"<color={GetColor(requirement.m_skill, requirement.m_level)}>{requirement.m_skillName}</color>: <color=orange>{requirement.m_level}</color>");
        }
        foreach (var requirement in data.m_equipRequirements.m_skillRequirements)
        {
            EquipRequirements.Add($"<color={GetColor(requirement.m_skill, requirement.m_level)}>{requirement.m_skillName}</color>: <color=orange>{requirement.m_level}</color>");
        }
        foreach (var requirement in data.m_consumeRequirements.m_skillRequirements)
        {
            ConsumeRequirements.Add($"<color={GetColor(requirement.m_skill, requirement.m_level)}>{requirement.m_skillName}</color>: <color=orange>{requirement.m_level}</color>");
        }

        if (CraftRequirements.Count > 0)
        {
            stringBuilder.Append("<color=yellow>Craft Requirements</color>\n");
        ...
        {
            stringBuilder.Append("<color=yellow>Equip Requirements</color>\n");
        ...
        {
            stringBuilder.Append("<color=yellow>Consume Requirements</color>\n");
    ...
    private static string GetColor(Skills.SkillType type, int requirement)
    {
        return GetPlayerSkill(type) < requirement ? "red" : "green";
    }

    private static string GetColor(int requirement, int level)
    {
        return level < requirement ? "red" : "green";
    }
    
    public static bool HaveRequirements(string name, bool itemName)
    {
        return CanCraft(name, itemName) || CanEquip(name, itemName) || CanConsume(name, itemName);
    }
    ...
    public static float GetPlayerSkill(Skills.SkillType type)
    {
        return !Player.m_localPlayer ? 0f : Player.m_localPlayer.GetSkillLevel(type);
    }
